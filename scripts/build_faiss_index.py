...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--id-column", default="tile_id")
    parser.add_argument("--index-type", default="ip", choices=["ip", "l2"])
    parser.add_argument("--output-index", required=True)
    parser.add_argument("--output-mapping-json", required=True)
...
def main() -> None:
    ...
    index = faiss.IndexFlatIP(features.shape[1]) if args.index_type == "ip" else faiss.IndexFlatL2(features.shape[1])
    index.add(features)
