#!/usr/bin/env python3
"""Evaluate predicted UAV orthophotos against UAV orthophoto truth crops.

Purpose:
- compute per-query quantitative alignment metrics on the common truth-grid
  domain shared by truth and predicted orthophotos;
- keep the formal accuracy conclusion anchored on UAV orthophoto truth rather
  than the DOM used during pose solving;
...
- truth and predicted orthophoto tiles on a shared grid.

Main outputs:
- `<output_root>/per_query_ortho_accuracy.csv`;
- `<output_root>/overall_ortho_accuracy.json`;
- `<output_root>/per_flight_ortho_accuracy.csv`;
- `<output_root>/failure_buckets.csv`.

Applicable task constraints:
- evaluate only the best pose per query;
- treat DOM overlays as diagnostic-only and keep them outside the main metric
  conclusion;
...
def main() -> None:
    ...
    for query_id in sorted(set(best_by_query) | set(truth_by_query) | set(pred_by_query)):
        ...
        if best_row is None:
            ...
            detail = "query missing from truth manifest"
            metrics = {}
    ...
    overall_payload = {
        ...
        "phase_corr_error_m": summarize_numeric([float(row["phase_corr_error_m"]) for row in ok_rows if row["phase_corr_error_m"] != ""]),
        "center_offset_m": summarize_numeric([float(row["center_offset_m"]) for row in ok_rows if row["center_offset_m"] != ""]),
        "ortho_iou": summarize_numeric([float(row["ortho_iou"]) for row in ok_rows if row["ortho_iou"] != ""]),
        "ssim": summarize_numeric([float(row["ssim"]) for row in ok_rows if row["ssim"] != ""]),
        "eval_status_counts": dict(status_counts),
        "generated_at_unix": time.time(),
    }

    per_flight_rows: list[dict[str, object]] = []
    for flight_id, rows in sorted(per_flight_metrics.items()):
        per_flight_rows.append(
            {
                "flight_id": flight_id,
                "query_count": len(rows),
                "phase_corr_error_m_mean": summarize_numeric([float(row["phase_corr_error_m"]) for row in rows if row["phase_corr_error_m"] != ""])["mean"],
                "center_offset_m_mean": summarize_numeric([float(row["center_offset_m"]) for row in rows if row["center_offset_m"] != ""])["mean"],
                "ortho_iou_mean": summarize_numeric([float(row["ortho_iou"]) for row in rows if row["ortho_iou"] != ""])["mean"],
                "ssim_mean": summarize_numeric([float(row["ssim"]) for row in rows if row["ssim"] != ""])["mean"],
            }
        )

    out_dir = eval_root
    write_csv(out_dir / "per_query_ortho_accuracy.csv", result_rows)
    write_json(out_dir / "overall_ortho_accuracy.json", overall_payload)
    write_csv(out_dir / "per_flight_ortho_accuracy.csv", per_flight_rows or [{
        "flight_id": "",
        "query_count": 0,
        "phase_corr_error_m_mean": "",
        "center_offset_m_mean": "",
        "ortho_iou_mean": "",
        "ssim_mean": "",
    }])
    write_csv(out_dir / "failure_buckets.csv", failure_rows or [{
        "query_id": "",
        "flight_id": "",
        "best_candidate_id": "",
        "failure_bucket": "",
    ...
    print(out_dir / "overall_ortho_accuracy.json")
