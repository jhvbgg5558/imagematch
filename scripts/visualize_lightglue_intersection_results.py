#!/usr/bin/env python3
"""Visualize LightGlue intersection rerank outputs and baseline comparisons."""
...
def plot_overall_compare(overall: dict, out_path: Path) -> None:
    ...
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Value")
    ax.set_title("Baseline vs LightGlue Intersection Metrics")
    ax.legend()
    for bars in (b1, b2):
        for bar in bars:
...
def plot_topk_upper_bound(overall: dict, out_path: Path) -> None:
    labels = ["Baseline R@10", "Coarse R@50", "LightGlue R@10"]
    vals = [
        overall["baseline_intersection_recall@10"],
    ...
    bars = ax.bar(np.arange(len(labels)), vals, color=[BASELINE_COLOR, UPPER_COLOR, LIGHTGLUE_COLOR])
    ax.set_xticks(np.arange(len(labels)), labels)
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Recall")
    ax.set_title("Top-50 Candidate Upper Bound vs Final Top-10")
...
def plot_top1_error_mean(overall: dict, out_path: Path) -> None:
    ...
    vals = [overall["baseline_top1_error_m_mean"], overall["lightglue_top1_error_m_mean"]]
...
def plot_multi_flight(per_flight_rows: list[dict[str, str]], out_path: Path) -> None:
    ...
    x = np.arange(len(labels))
    width = 0.25
    r1 = [float(row["lightglue_intersection_recall@1"]) for row in per_flight_rows]
    r5 = [float(row["lightglue_intersection_recall@5"]) for row in per_flight_rows]
    r10 = [float(row["lightglue_intersection_recall@10"]) for row in per_flight_rows]
    ...
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Recall")
    ax.set_title("LightGlue Intersection Multi-Flight Recall")
    ax.legend()
...
def plot_rank_delta(comp_rows: list[dict[str, str]], flight_id: str, out_path: Path) -> None:
    ...
    colors = []
    for row in rows:
        b = row["baseline_first_truth_rank"]
        l = row["lightglue_first_truth_rank"]
        b_rank = 51 if b == "" else int(b)
        l_rank = 51 if l == "" else int(l)
...
def main() -> None:
    ...
    tile_meta = {row["tile_id"]: row for row in load_csv(Path(args.tiles_csv))}
    ...
    plot_topk_upper_bound(overall, figure_dir / "_aggregate" / "top50_upper_bound.png")
    ...
    for row in per_flight_rows:
        ...
        summary = load_json(lightglue_dir / "stage7" / flight_id / f"rerank_top{args.rerank_top_k}.json")
        reranked = load_reranked(lightglue_dir / "stage7" / flight_id / f"reranked_top{args.rerank_top_k}.csv")
        ...
        labels = ["R@1", "R@5", "R@10", "MRR"]
        vals = [
            float(row["lightglue_intersection_recall@1"]),
            float(row["lightglue_intersection_recall@5"]),
            float(row["lightglue_intersection_recall@10"]),
            float(row["lightglue_intersection_mrr"]),
        ]
        ...
        ax.set_ylim(0, 1.05)
        ax.set_ylabel("Value")
        ax.set_title(f"Flight {row['flight_tag']} Intersection Metrics")
        for bar, value in zip(bars, vals):
            ax.text(bar.get_x() + bar.get_width() / 2, value + 0.02, f"{value:.3f}", ha="center", fontsize=9)
        fig.tight_layout()
