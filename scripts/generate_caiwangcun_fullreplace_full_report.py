...
def build_markdown(data: dict[str, Any], assets: dict[str, Path], out_md: Path, missing: list[str]) -> str:
    ...
    validation_rows = [
        ...
        ["Layer-3 per-query tiepoint CSV", f"{detail['csv_count']}/{detail['expected_ok_count']} ok-query files"],
        ["Layer-3 missing detail query", ", ".join(detail["failed_ids"] + detail["missing_ok_ids"]) or "-"],
