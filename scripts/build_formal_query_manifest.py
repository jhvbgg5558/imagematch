...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--intrinsics-fallback-csv", default=str(DEFAULT_INTRINSICS_FALLBACK_CSV))
...
def load_intrinsics_fallback(path: Path) -> dict[tuple[str, str], dict[str, object]]:
    if not path.exists():
        return {}
    rows = load_csv(path)
    fallback: dict[tuple[str, str], dict[str, object]] = {}
    for row in rows:
        key = (row["flight_id"], row["image_name"])
        fallback[key] = {
            "intrinsics_model": row["intrinsics_model"],
            "width_px": int(float(row["width_px"])),
            "height_px": int(float(row["height_px"])),
            "fx_px": float(row["fx_px"]),
            "fy_px": float(row["fy_px"]),
            ...
            "p1": float(row["p1"]),
            "p2": float(row["p2"]),
            "source": row.get("intrinsics_source", str(path)),
        }
    return fallback


def apply_query_downsample_overrides(
    intr: dict[str, object], row: dict[str, str]
) -> tuple[dict[str, object], dict[str, object]]:
    if not row.get("downsampled_width_px") or not row.get("downsampled_height_px"):
        return intr, {}
    out = dict(intr)
    original_width = float(row.get("original_width_px") or intr["width_px"])
    original_height = float(row.get("original_height_px") or intr["height_px"])
    width = int(float(row["downsampled_width_px"]))
    height = int(float(row["downsampled_height_px"]))
    scale_x = float(row.get("intrinsics_scale_x") or (width / original_width))
    scale_y = float(row.get("intrinsics_scale_y") or (height / original_height))
    out["width_px"] = width
    out["height_px"] = height
    out["fx_px"] = float(intr["fx_px"]) * scale_x
    out["fy_px"] = float(intr["fy_px"]) * scale_y
    out["cx_px"] = float(intr["cx_px"]) * scale_x
    out["cy_px"] = float(intr["cy_px"]) * scale_y
    return out, {
        "downsample_target_gsd_m": row.get("downsample_target_gsd_m", ""),
        "estimated_query_gsd_m": row.get("estimated_query_gsd_m", ""),
        "downsample_scale": row.get("downsample_scale", ""),
...
def main() -> None:
    ...
    intrinsics_fallback = load_intrinsics_fallback(Path(args.intrinsics_fallback_csv))
    ...
    for row in query_rows:
        ...
        elif key in intrinsics_fallback:
            intr = intrinsics_fallback[key]
            intrinsics_source = str(intr.get("source", Path(args.intrinsics_fallback_csv))).replace("\\", "/")
        else:
            raise SystemExit(f"cameras.json not found for query pair {key}: {cameras_json}")
        intr, downsample_meta = apply_query_downsample_overrides(intr, row)
        if downsample_meta:
            intrinsics_source = f"{intrinsics_source}|query_manifest_downsample"
        output_row = {
        ...
        if downsample_meta:
            output_row.update(downsample_meta)
