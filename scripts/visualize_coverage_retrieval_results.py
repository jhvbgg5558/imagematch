#!/usr/bin/env python3
"""Visualize single-run coverage-truth retrieval results."""
...
def configure_fonts() -> None:
    # Keep matplotlib defaults for charts because all chart text is ASCII.
    # This avoids glyph fallback issues in environments with partial CJK fonts.
    plt.rcParams["font.sans-serif"] = ["DejaVu Sans"]
    plt.rcParams["axes.unicode_minus"] = False


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize coverage-truth retrieval outputs.")
...
def per_flight_metrics(summary: dict, seed_rows: dict[str, dict[str, str]]) -> dict[str, dict[str, float]]:
    ...
    for flight_id, rows in grouped.items():
        ...
        out[flight_id] = {
            "coverage_recall@1": sum(int(row["coverage_hit@1"]) for row in rows) / total if total else 0.0,
            "coverage_recall@5": sum(int(row["coverage_hit@5"]) for row in rows) / total if total else 0.0,
            "coverage_recall@10": sum(int(row["coverage_hit@10"]) for row in rows) / total if total else 0.0,
            "coverage_mrr": sum(float(row["coverage_reciprocal_rank"]) for row in rows) / total if total else 0.0,
...
def plot_metrics_bar(metrics: dict[str, float], title: str, out_path: Path) -> None:
    labels = ["Coverage R@1", "Coverage R@5", "Coverage R@10", "Coverage MRR"]
    values = [
        metrics["coverage_recall@1"],
        metrics["coverage_recall@5"],
        metrics["coverage_recall@10"],
        metrics["coverage_mrr"],
    ]
...
def plot_query_rr(summary: dict, seed_rows: dict[str, dict[str, str]], flight_id: str, out_path: Path) -> None:
    ...
    vals = [float(row["coverage_reciprocal_rank"]) for row in rows]
    fig, ax = plt.subplots(figsize=(10, 4.8))
    bars = ax.bar(qids, vals, color="#1f77b4")
    ax.set_ylim(0, 1.05)
    ax.set_ylabel("Coverage Reciprocal Rank")
...
def save_query_contact_sheet(
    ...
    images = [
        labeled_thumb(
            query_path,
            [query_id, srow["flight_id"].split("_")[2], "coverage truth"],
            QUERY_BORDER,
        )
    ]
...
def plot_overall_metrics(summary: dict, out_path: Path) -> None:
    labels = ["Coverage R@1", "Coverage R@5", "Coverage R@10", "Coverage MRR"]
    values = [
        float(summary["coverage_recall@1"]),
        float(summary["coverage_recall@5"]),
        float(summary["coverage_recall@10"]),
        float(summary["coverage_mrr"]),
    ]
    fig, ax = plt.subplots(figsize=(8, 4.8))
    bars = ax.bar(np.arange(len(labels)), values, color=METRIC_COLORS)
    ax.set_xticks(np.arange(len(labels)), labels)
    ax.set_ylim(0, 1.1)
    ax.set_ylabel("Value")
    ax.set_title(f"Coverage Metrics | Top1Err={float(summary['top1_error_m_mean']):.1f}m")
    for bar, value in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width() / 2, value + 0.02, f"{value:.3f}", ha="center", fontsize=9)
    fig.tight_layout()
    fig.savefig(out_path, dpi=180)
...
def plot_center_metrics(summary: dict, out_path: Path) -> None:
    labels = ["Center R@1", "Center R@5", "Center R@10"]
    values = [
        float(summary["center_recall@1"]),
        float(summary["center_recall@5"]),
        float(summary["center_recall@10"]),
    ]
    fig, ax = plt.subplots(figsize=(7, 4.8))
    bars = ax.bar(np.arange(len(labels)), values, color=METRIC_COLORS[:3])
    ax.set_xticks(np.arange(len(labels)), labels)
    ax.set_ylim(0, 1.1)
    ax.set_ylabel("Value")
    ax.set_title("Center Metrics")
    for bar, value in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width() / 2, value + 0.02, f"{value:.3f}", ha="center", fontsize=9)
    fig.tight_layout()
    fig.savefig(out_path, dpi=180)
...
def plot_multi_flight_recall(flight_metrics: dict[str, dict[str, float]], out_path: Path) -> None:
    ...
    r1 = [flight_metrics[f]["coverage_recall@1"] for f in flights]
    r5 = [flight_metrics[f]["coverage_recall@5"] for f in flights]
    r10 = [flight_metrics[f]["coverage_recall@10"] for f in flights]
    ...
    ax.set_title("Coverage Multi-Flight Recall")
    ax.legend()
...
def plot_top1_error_distribution(summary: dict, out_path: Path) -> None:
    ...
    colors = [HIT_BORDER if row["coverage_hit@1"] else MISS_BORDER for row in rows]
    ...
    for bar, value in zip(bars, vals):
        ax.text(bar.get_x() + bar.get_width() / 2, value + max(vals) * 0.01, f"{value:.0f}", ha="center", fontsize=7, rotation=90)
    fig.tight_layout()
    fig.savefig(out_path, dpi=180)
    plt.close(fig)
...
def main() -> None:
    ...
    truth_tiles = load_truth_tiles(result_dir / "query_truth" / "query_truth_tiles.csv")
    retrieval = load_retrieval(result_dir / "retrieval" / "retrieval_top10.csv")
    ...
    plot_center_metrics(summary, aggregate_dir / "center_metrics_bar.png")
