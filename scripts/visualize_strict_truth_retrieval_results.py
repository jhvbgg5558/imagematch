#!/usr/bin/env python3
"""Visualize single-run strict-truth retrieval results."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize strict-truth retrieval outputs.")
...
def per_flight_metrics(summary: dict, seed_rows: dict[str, dict[str, str]]) -> dict[str, dict[str, float]]:
    ...
    for flight_id, rows in grouped.items():
        ...
        out[flight_id] = {
            "strict_recall@1": sum(int(row["strict_hit@1"]) for row in rows) / total if total else 0.0,
            "strict_recall@5": sum(int(row["strict_hit@5"]) for row in rows) / total if total else 0.0,
            "strict_recall@10": sum(int(row["strict_hit@10"]) for row in rows) / total if total else 0.0,
            "strict_mrr": sum(float(row["strict_reciprocal_rank"]) for row in rows) / total if total else 0.0,
...
def plot_metrics_bar(metrics: dict[str, float], title: str, out_path: Path) -> None:
    ...
    values = [
        metrics["strict_recall@1"],
        metrics["strict_recall@5"],
        metrics["strict_recall@10"],
        metrics["strict_mrr"],
    ]
...
def plot_query_rr(summary: dict, seed_rows: dict[str, dict[str, str]], flight_id: str, out_path: Path) -> None:
    ...
    ax.set_title(f"Flight {flight_tag} Strict Selection Scores")
...
def save_query_contact_sheet(
    ...
    images = [
        labeled_thumb(
            query_path,
            [query_id, srow["flight_id"].split("_")[2], "strict truth"],
            QUERY_BORDER,
        )
    ]
...
def plot_overall_metrics(summary: dict, out_path: Path) -> None:
    labels = ["Strict R@1", "Strict R@5", "Strict R@10", "Strict MRR"]
    values = [
        float(summary["strict_recall@1"]),
        float(summary["strict_recall@5"]),
        float(summary["strict_recall@10"]),
        float(summary["strict_mrr"]),
    ]
    fig, ax = plt.subplots(figsize=(8, 4.8))
    bars = ax.bar(np.arange(len(labels)), values, color=METRIC_COLORS)
    ax.set_xticks(np.arange(len(labels)), labels)
    ax.set_ylim(0, 1.1)
    ax.set_ylabel("Value")
    ax.set_title(f"Strict Truth Metrics | Top1Err={float(summary['top1_error_m_mean']):.1f}m")
    for bar, value in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width() / 2, value + 0.02, f"{value:.3f}", ha="center", fontsize=9)
    fig.tight_layout()
    fig.savefig(out_path, dpi=180)
...
def plot_center_metrics(summary: dict, out_path: Path) -> None:
    labels = ["Center-Strict R@1", "Center-Strict R@5", "Center-Strict R@10"]
    values = [
        float(summary["center_strict_recall@1"]),
        float(summary["center_strict_recall@5"]),
        float(summary["center_strict_recall@10"]),
    ]
    ...
    ax.set_ylim(0, 1.1)
    ax.set_ylabel("Value")
    ax.set_title("Center-Strict Metrics")
    for bar, value in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width() / 2, value + 0.02, f"{value:.3f}", ha="center", fontsize=9)
    fig.tight_layout()
    fig.savefig(out_path, dpi=180)
...
def plot_multi_flight_recall(flight_metrics: dict[str, dict[str, float]], out_path: Path) -> None:
    ...
    r1 = [flight_metrics[f]["strict_recall@1"] for f in flights]
    r5 = [flight_metrics[f]["strict_recall@5"] for f in flights]
    r10 = [flight_metrics[f]["strict_recall@10"] for f in flights]
    ...
    ax.set_title("Strict Truth Multi-Flight Recall")
    ax.legend()
...
def plot_top1_error_distribution(summary: dict, out_path: Path) -> None:
    ...
    colors = [HIT_BORDER if row["strict_hit@1"] else MISS_BORDER for row in rows]
