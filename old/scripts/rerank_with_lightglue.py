#!/usr/bin/env python3
"""Rerank coarse retrieval candidates with SuperPoint/ALIKED + LightGlue."""
...
def build_extractor(name: str, max_num_keypoints: int, device: str):
    if name == "aliked":
...
def verify_pair(
    feats_q: dict[str, torch.Tensor],
    feats_c: dict[str, torch.Tensor],
    matcher,
...
def main() -> None:
    ...
    extractor, matcher = build_extractor(args.feature_backend, args.max_num_keypoints, args.device)
    ...
    with metrics_csv.open("w", newline="", encoding="utf-8") as mf, output_csv.open(
        ...
        for query_id, query_meta in queries.items():
            if query_id not in query_feat_cache:
                query_feat_cache[query_id] = extract_features(
                    read_rgb(Path(str(query_meta["image_path"]))), extractor, args.device
                )
            ...
            for item in retrieval.get(query_id, []):
                ...
                if tile_id not in tile_feat_cache:
                    tile_feat_cache[tile_id] = extract_features(
                        read_rgb(Path(str(tile_meta["image_path"]))), extractor, args.device
                    )
            ...
            for cand in candidates:
                metrics_writer.writerow(
                    [
                        ...
                        cand["keypoints_query"],
                        cand["keypoints_candidate"],
                        cand["match_count"],
                        cand["inlier_count"],
                        cand["inlier_ratio"],
