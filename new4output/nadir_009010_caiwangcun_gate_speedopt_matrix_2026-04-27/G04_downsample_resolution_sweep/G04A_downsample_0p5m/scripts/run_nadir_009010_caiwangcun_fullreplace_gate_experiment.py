...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Optional candidate DOM tile GSD for resolution sweeps. Values <=0 keep native output.",
    )
    parser.add_argument(
        "--query-output-gsd-m",
        type=float,
        default=0.0,
        help="Optional query image GSD for resolution sweeps. Values <=0 keep reused query images/features.",
    )
    ...
    parser.add_argument("--geometry-matcher", choices=["romav2", "siftgpu"], default="romav2")
    ...
    parser.add_argument("--romav2-export-pose-matches", action="store_true")
    parser.add_argument("--reuse-rerank-pose-matches", action="store_true")
    parser.add_argument("--domz-sampling", action="store_true")
    parser.add_argument("--dsm-workers", type=int, default=1)
...
def main() -> None:
    ...
    run_cmd(
        ...
        + (["--output-gsd-m", str(args.candidate_output_gsd_m)] if args.candidate_output_gsd_m > 0 else []),
        log_path,
        args.dry_run,
    )
    ...
    if args.query_output_gsd_m > 0:
        tag = str(args.query_output_gsd_m).replace(".", "p")
        run_cmd(
            [
                args.python_bin,
                str(PROJECT_ROOT / "scripts" / "downsample_query_inputs_for_resolution_sweep.py"),
                "--query-manifest-csv",
                str(experiment_root / "query_inputs" / "query_manifest.csv"),
                "--query-seed-csv",
                str(query_seed_csv),
                "--out-query-manifest-csv",
                str(experiment_root / "query_inputs" / "query_manifest.csv"),
                "--out-image-root",
                str(experiment_root / "query_inputs" / f"images_downsampled_{tag}m"),
                "--summary-json",
                str(experiment_root / "query_inputs" / f"query_downsample_{tag}m_summary.json"),
                "--target-gsd-m",
                str(args.query_output_gsd_m),
            ],
            log_path,
            args.dry_run,
        )
        run_cmd(
            [
                ...
                "--output-csv",
                str(experiment_root / "query_features" / "query_dinov2_status.csv"),
                "--model-name",
    ...
    geom_root = experiment_root / ("siftgpu_rerank" if args.geometry_matcher == "siftgpu" else "romav2_rerank")
    if not args.skip_retrieval:
        ...
        run_cmd(
            [
                ...
                str(args.top_k),
                "--output-csv",
                str(geom_root / "coarse" / f"retrieval_top{args.top_k}.csv"),
                "--summary-json",
                str(geom_root / "coarse" / f"summary_top{args.top_k}.json"),
                "--curve-csv",
                str(geom_root / "coarse" / f"topk_truth_curve_top{args.top_k}.csv"),
            ],
            log_path,
            args.dry_run,
        )
        ...
        run_cmd(
            [
                args.python_bin,
                str(PROJECT_ROOT / "scripts" / ("run_siftgpu_rerank_intersection_round.py" if args.geometry_matcher == "siftgpu" else "run_romav2_rerank_intersection_round.py")),
                "--input-round-root",
                str(geom_root / "input_round"),
                "--tiles-csv",
                str(tiles_csv),
                ...
                "inlier_count_only",
                "--timing-json",
                str(geom_root / "timing" / ("siftgpu_rerank_internal.json" if args.geometry_matcher == "siftgpu" else "romav2_rerank_internal.json")),
            ]
            + (
                ["--colmap-bin", args.colmap_bin, "--max-num-features", str(args.siftgpu_max_num_features)]
                + (["--siftgpu-bin", args.siftgpu_bin] if args.siftgpu_bin else [])
                if args.geometry_matcher == "siftgpu"
                else ["--device", args.device, "--sample-count", str(args.romav2_sample_count)]
            )
            + (["--export-pose-matches"] if args.romav2_export_pose_matches else [])
    ...
    if not args.skip_pose:
        ...
        reuse_match_csv = bundle_root / "matches" / "roma_matches_reused_from_rerank.csv"
        if args.reuse_rerank_pose_matches:
            run_cmd(
                [
                    args.python_bin,
                    ...
                    str(reuse_match_csv),
                    "--summary-json",
                    str(bundle_root / "matches" / "roma_matches_reused_from_rerank.summary.json"),
                ]
                + (
                    ["--input-name", "siftgpu_matches_for_pose.csv", "--stage-name", "merge_siftgpu_pose_matches"]
                    if args.geometry_matcher == "siftgpu"
                    else []
                )
                + [item for query_id in args.rerank_query_id for item in ("--query-id", query_id)],
        ...
        if args.reuse_rerank_pose_matches:
            formal_pose_cmd.extend(["--reuse-match-csv", str(reuse_match_csv)])
        if args.domz_sampling:
            formal_pose_cmd.extend(["--sampling-mode", "domz", "--domz-workers", str(args.domz_workers)])
        run_cmd(
            formal_pose_cmd,
            log_path,
            args.dry_run,
