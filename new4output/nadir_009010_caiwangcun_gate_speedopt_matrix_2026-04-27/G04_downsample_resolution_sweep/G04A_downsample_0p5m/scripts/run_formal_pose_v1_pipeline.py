...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reuse-match-csv", default=None)
    parser.add_argument("--dsm-workers", type=int, default=1)
    parser.add_argument("--sampling-mode", choices=("dsm", "domz"), default="dsm")
    parser.add_argument("--domz-cache-csv", default=None)
...
def reuse_match_csv(source_path: Path, target_path: Path) -> dict[str, object]:
    started = time.time()
    ensure_dir(target_path.parent)
    if source_path.resolve() != target_path.resolve():
        shutil.copy2(source_path, target_path)
    return {
        "command": ["reuse_match_csv", str(source_path), str(target_path)],
        "returncode": 0,
        "stdout_tail": str(target_path),
        "stderr_tail": "",
        "elapsed_seconds": time.time() - started,
    }
...
def write_stage_summary(
    ...
    write_json(
        bundle_root / "summary" / "phase_gate_summary.json",
        {
...
def main() -> None:
    ...
    if not args.skip_dsm_build:
        ...
        if int(stage_runs[-1]["returncode"]) != 0:
            write_stage_summary(bundle_root, args.phase, query_ids, stage_runs)
            raise SystemExit(int(stage_runs[-1]["returncode"]))

    matches_path = bundle_root / "matches" / "roma_matches.csv"
    if args.reuse_match_csv:
        reuse_source = Path(args.reuse_match_csv)
        if not reuse_source.exists():
            raise SystemExit(f"--reuse-match-csv does not exist: {reuse_source}")
        stage_runs.append({"stage": "reuse_romav2_matches_for_pose", **reuse_match_csv(reuse_source, matches_path)})
    else:
        roma_cmd = [
            args.python_exe,
    ...
    if int(stage_runs[-1]["returncode"]) != 0:
        write_stage_summary(bundle_root, args.phase, query_ids, stage_runs)
        raise SystemExit(int(stage_runs[-1]["returncode"]))
    if args.sampling_mode == "domz":
        domz_cache_csv = Path(args.domz_cache_csv) if args.domz_cache_csv else bundle_root / "domz_cache" / "domz_point_cache.csv"
        if args.domz_cache_csv is None:
            stage_runs.append(
                {
                    ...
                    **run_step(
                        [
                            args.python_exe,
                            str(PROJECT_ROOT / "scripts" / "build_domz_point_cache_for_correspondences.py"),
                            "--bundle-root",
                            str(bundle_root),
                            "--manifest-json",
                            str(manifest_path),
        ...
        stage_runs.append(
            {
                ...
                **run_step(
                    [
                        args.python_exe,
                        str(PROJECT_ROOT / "scripts" / "sample_domz_cache_for_dom_points.py"),
                        "--bundle-root",
                        str(bundle_root),
                        "--correspondences-csv",
                        str(bundle_root / "correspondences" / "pose_correspondences.csv"),
